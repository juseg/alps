...
\newenvironment{mask}{\color{gray}}{\color{black}}
%TC:newcounter mask
%TC:envir mask [] mask
...
\title{Supercomputing shows rapid Alpine palaeo-glacier change\\
       \small\note{is this too basic? I have other ideas.}}
...
% Texcount template
%TC:newtemplate
%TC:template Word counts (text (masked), headers, other):\n
%TC:template  {SUB?* {title}: {text} ({mask}), {headerword}, {otherword}\n?SUB}
%TC:template Total: {text} ({mask}), {headerword}, {otherword}

\begin{framed}
\small\verbatiminput{alps-cycle.sum}
\end{framed}
...
\begin{abstract}
    ...
    glacial cycle (120--0\,ka) in the Alps, using 576~processors and a
...
% ----------------------------------------------------------------------
    ...
    indicating tens of glacial and interglacial periods. During the last
    800\,000 years before present (800\,ka), these \emph{glacial cycles}
    ...
    \citep[e.g.,][]{Kleman.1994, Kleman.etal.2010, Fabel.etal.2012}. Although
...
% ----------------------------------------------------------------------
    ...
    and 20\,m, respectively. This simulation ran for 33~days using
    576~processors.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    detail across the Western \citep{Coutterand.2010,
    Buoncristiani.Campy.2011}, Central \citep{Jackli.1962, Bini.etal.2009,
    Hantke.2011}, and Eastern Alps \citep{Penck.Bruckner.1909, Husen.1987,
    BGR.2007, Husen.2011, Bavec.Verbic.2011}.
    Some of these maps have been compiled
    ...
    from introducing a time-dependent palaeo-temperature forcing. Constant
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    Pyhrn Pass \citep[Fig.~2.5]{Husen.2011}.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    anomaly gradients, not included in our model set-up, have perhaps induced
    further spatial differences in the timing of the LGM Alpine ice sheet,
    which may have counterbalanced or enhanced those modelled here.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    However, no evidence for cold-based glaciation has yet been reported in the
    Alps.
...
% ----------------------------------------------------------------------
    ...
    \begin{itemize}
      ...
      \item The EPICA palaeo-temperature forcing yields cumulative ice extent
            ...
            both GRIP and MD01-2444 forcing result in MIS~4 glaciation well
    ...
    flow physics. In particular, additional palaeo-climate variability over
    Central Europe may have caused more glaciations than modelled here.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    \citep[Table~\ref{tab:params};][]{Aschwanden.etal.2012} and a piece-wise
    Arrhenius-type rheology representative of Holocene polar ice
    \citep[Table~\ref{tab:params};][p.~77]{Cuffey.Paterson.2010}.
    The englacial water fraction is
    capped at a~maximum value of 0.01, above which no measurements are
    available \citep[Eq.~5.7]{Lliboutry.Duval.1985, Greve.1997}.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    criterion using a constant basal friction angle (Table~\ref{tab:params}).
    ...
    Antarctica \citep[Table~\ref{tab:params};][]{Tulaczyk.etal.2000,
    Bueler.Pelt.2015}. When the till becomes saturated, additional melt water
    is assumed to drain off instantaneously.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    \citep{Mey.etal.2016}, our simulations effectively use an elastic thickness
    of the lithosphere of $E=52.4$\,km, instead of the 50\,km value used by
    \citet{Mey.etal.2016}, introducing in a small change in the lenght scale of
    bedrock deformation well within uncertainties \citep{Mey.etal.2016}.}
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    (Fig.~\ref{fig:inputs}h). WorldClim data
...
% ----------------------------------------------------------------------
    ...
    40\,m, respectively. Simulations ran for two to four days
    using 144~processors.
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    that the modelled cumulative glaciated area during MIS~2 (29--14\,ka)
    and within a rectangular region covering the Rhine
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    the upper Inn Valley \citep{Barrett.etal.2017}. For the Reuss Glacier, no
...
\codedataavailability{%
    ...
    through The Cryosphere. Please contact the corresponding author for mode
...
% ----------------------------------------------------------------------
    ...
    \begin{figure*}
      ...
      \caption{%
        ...
        cover. Modelled self-sustained ice domes (triangles) and some major
        transfluences (crosses) are shown. Modelled LGM
        ice extent (dashed orange line) and geomorphological
    ...
    \begin{figure*}
      \centerline{\includegraphics{alpcyc_hr_dynamics}}
      \caption{%
        ...
        corresponding, ice surface elevation (200\,m contours). Observed upper
        Rhone valley trimline locations \citep[black dots;][Table~1]
        {Kelly.etal.2004} and areas modelled to have experienced
        less than 1\,ka temperate ice cover during MIS~2 (29--14\,ka, hatches).
        The LGM is here modelled as a time-transgressive event. Profile lines
        roughly following valley centerlines for the Rhine and Inn Glaciers.
        \textbf{(b)} Comparison of modelled ice surface elevation at the LGM
        ...
        \textbf{(c)} Evolution of modelled glacier extent in
        time, bilinearly interpolated along the Rhine and Inn profiles, showing
...
%TC:break Supplementary figures
% ----------------------------------------------------------------------
...
% ----------------------------------------------------------------------
    ...
    \begin{figure*}
      ...
      \caption{%
        ...
        \textbf{(c)} Extract from the estimated of present-day ice thickness
        \citep{Huss.Farinotti.2012} substracted from the SRTM topography.
        ...
        \citep[1979--2012;][]{Dee.etal.2011} from the reference monthly
        climatology used to force the surface mass balance (PDD) component of
        the ice sheet model.
        \textbf{(f)} Modern January and \textbf{(g)} July mean near-surface air
        temperature, and \textbf{(h, i)} precipitation from WorldClim
    ...
    \begin{figure*}
      ...
      \caption{%
        ...
        \chem{\delta^{18}O} records \citep{Lisiecki.Raymo.2005}. The simulation
