...
\label{sec:atm}%
    ...
    \idea{move paleo-climate forcing info to the next section.}

    Climate forcing driving ice sheet simulations consists of a~present-day
    monthly climatology, $\{T_{\mathrm{m}0}, P_{\mathrm{m}0}\}$, where
    temperatures are modified by offset time series, ${\Delta}T_{\text{TS}}$,
    and lapse-rate corrections, ${\Delta}T_{\text{LR}}$:
%
\begin{align}
...
%
    ...
    other hand. All simulations use an annual temperature lapse rate of $\gamma
    = 6\,\unit{K\,km^{-1}}$.
...
P_{\mathrm{m}}(t, x, y) = P_{\mathrm{m}0}(x, y) \cdot
                          \exp[\zeta{\Delta}T_{\text{LR}}(t, x, y)] \,.
\end{align}
%
    with $\zeta=0.169/2.4=0.0704$ \citep{Huybrechts.2002}. This simple
    ...
    precipitation, corresponding to $\zeta=0$. In the rest of this paper, we
    ...
    corrections are identified by appending ${\Delta}P$ to their name.
...
\subsection{Palaeo-precipitation forcing}
    ...
    \begin{itemize}
    \item Some simulations use reduced precipitation (Table~\ref{tab:records}).
    \end{itemize}
