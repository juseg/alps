...
\begin{abstract}
    ...
    at low elevation, \ian{I am not sure I understand this phrase: \textit{while the intra-montane glacial landscape is modelled to
    ...
    periods, thus offering insight into its diversity \ian{Diversity of what? I think the
    sentence should stop at the comma. }.
...
% ----------------------------------------------------------------------
    ...
    the Alps, present a higher variety of glacier erosional landforms \ian{added
      these old refs... but maybe you can read the one in French better than me 
    \citep[][(Fig.~\ref{fig:landscape})]{Penck.1905,Agassiz.2012},}
    whose implications on glacial history is yet to be understood.

    In absence of other landforms, glacially eroded topography has sometimes
    been used as a proxy for glacier extension \ian{extent}, for instance in
    ...
    transition to cold-based ice is, and why some glaciated regions show \ian{
    ...
    On longer time scales \ian{I might remove ``on longer timescales''. it seems
    that the processes likely occur concurrently}, glacier erosion \ian{also} likely governs, at least in part,
    ...
    erosion, but yielding much higher erosion rates \ian{I might be reading this
    sentence wrong, but higher erosion rates compared to what?}.
    ...
    patterns of modelled erosion rate\ian{s} and cumulative last glacial cycle erosion
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    Our approach does not account for \ian{ feedbacks} of glacier erosion onto
    bedrock topography and, then onto, ice dynamics
    \citep[e.g.,][]{Anderson.etal.2012}. \ian{Furthermore, we assume that
      once material is eroded, it is transported out of the system. In turn, we
      do not account for the downstream advection of debris
      \citep[e.g.,][]{Anderson.Anderson.2016}, or fluvial
      transport of subglacial sediment \citep{Delaney.etal.2019}.}

Instead, we compute the time-integrated
    ...
    \ian{Remove?: \textit{It
    can be noted  from } We note that in} the above formula, that with an exponent, $l$,
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    cirques are poorly captured by the model\ian{'s} horizontal resolution, high
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    \begin{figure*}[ht]
      ...
      \caption{%
        \textbf{(a)} Modelled erosion rate ``hypsogram'', showing the geometric
          ...
          across the entire model domain and its time evolution. \ian{ the black bit
            is cumulative erosion through the time period, right? maybe worth a clarifying word. }
    ...
    to be moderate \ian{moderate? around 1 mm a$^{-1}$} and restricted the lower parts of the catchment, while much
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    erosion \ian{armored the glacier bed from erosion?} for at least parts
    of the glacial cycle. \ian{remove? \textbf{Finally, all empirical erosion laws are affected
        with very large uncertainties.}}
    \ian{Instead? \textit{Given the lack of data to constrain the erosion rule, }}it can not
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    \ian{This result may seem paradoxical, and is modulated in a limited extent
      by the exponent, $l$, in the erosion rule.
      However, it is in fact compatible with previous field-based studies that
  ...
  While this may seem paradoxical, and is modulated by the exponent, $l$, in the erosion
    power-law, it is in fact compatible with previous field-based studies that
    have evidenced an increase of erosion rates with air temperatures for at
    least some sets of glaciers \citep{Koppes.etal.2015, Cook.etal.2020}.

    Cold temperatures generally reduce sliding  and if  the glacier becomes
    cold-based then sliding, and thus erosion, will cease altogether
    \citep{Koppes.etal.2015}. In cases of glacier advance, newly formed, atmosphere-cold ice
    ...
    \ian{Maybe your field, but are there some papers that can reference the
      polythermal processes, or specifically reference that the model includes
      these processes?}
    ...
    erosion and pro-glacial sedimentation during the \ian{current} deglaciation period
    \citep[e.g.,][]{Koppes.Montgomery.2009, Micheletti.Lane.2016,
    Lane.etal.2017, Bendixen.etal.2017},\ian{remove?: \textbf{including the ongoing glacier retreat
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    \citep{Imhof.etal.2019}, the lack \ian{of rapid erosion at } high-elevations  for much
...
% ----------------------------------------------------------------------
    ...
    Due to stacked-up \ian{compounded?} uncertainties regarding paleoclimate, glacier sliding and
    erosion processes, our quantitative results are very likely inaccurate.
    \ian{remove? \textbf{    Yet to the best of our current knowledge in these respective areas of
    the geosciences,} Yet, we can \ian{remove can?} draw the following qualitative conclusions:
    \begin{itemize}
